...
type IrrigationSite={id:string;project_id:string;name:string;code:string|null;district:string|null};
type IrrigationProject={id:string;name:string;sites:IrrigationSite[]};
type LocalSite={id:string;project_id:string;source_site_id:string|null};
type ManualOrder={number:string;startDate:string;endDate:string;irrigationProjectId:string;siteIds:string[];quantities:Record<string,string>};
const keyOf=(itemNo:string,itemName:string)=>`${itemNo}::${normalizeArabic(itemName)}`;
const emptyOrder=(index:number,irrigationProjectId=''):ManualOrder=>({number:String(index+1).padStart(2,'0'),startDate:'',endDate:'',irrigationProjectId,siteIds:[],quantities:{}});
...
export default function ImportPage(){
  ...
  useEffect(()=>{void loadIrrigationCatalog()},[]);

  async function loadIrrigationCatalog(){
    setCatalogLoading(true);setCatalogError('');
    try{const response=await fetch(`/api/irrigation/catalog?t=${Date.now()}`,{cache:'no-store'});const payload=await response.json();if(!response.ok)throw new Error(payload.error||'تعذر جلب دليل مشاريع الري.');setIrrigationProjects(payload.projects||[]);}
    catch(error){setCatalogError(error instanceof Error?error.message:'تعذر جلب دليل مشاريع الري.');setIrrigationProjects([]);}
  ...
  function setOrderCount(value:number){
    ...
    const defaultProjectId=irrigationProjects.length===1?irrigationProjects[0].id:'';
    setOrders(current=>Array.from({length:count},(_,index)=>current[index]||emptyOrder(index,defaultProjectId)));
  ...
  function chooseIrrigationProject(_index:number,projectId:string){
    // ملف Excel الواحد يمثل مشروعًا محليًا واحدًا، لذلك يكون مشروع الري
    // موحدًا لكل الأوامر مع بقاء اختيار المواقع مستقلًا لكل أمر.
    setOrders(current=>current.map(order=>({
      ...order,
      irrigationProjectId:projectId,
      siteIds:order.irrigationProjectId===projectId?order.siteIds:[],
    })));
  }
  ...
  const validation=useMemo(()=>orders.map(order=>({
    ready:Boolean(order.number.trim()&&order.startDate&&order.endDate&&order.endDate>=order.startDate&&order.irrigationProjectId&&order.siteIds.length&&Object.values(order.quantities).some(value=>Number(value)>0)),
  ...
  const activeIrrigationProject=useMemo(()=>irrigationProjects.find(project=>project.id===active?.irrigationProjectId)||null,[irrigationProjects,active?.irrigationProjectId]);
  ...
  async function approve(){
    ...
    try{
      const irrigationProjectIds=[...new Set(orders.map(order=>order.irrigationProjectId).filter(Boolean))];
      if(irrigationProjectIds.length!==1)throw new Error('يجب أن تكون جميع أوامر العمل ضمن مشروع ري واحد لأن ملف Excel يمثل مشروعًا واحدًا.');
      const irrigationProjectId=irrigationProjectIds[0];
      const projectId=await ensureProject(data);const contractId=await ensureContract(projectId,data);
      const linkResponse=await fetch('/api/irrigation/link',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({workProjectId:projectId,irrigationProjectId})});const linkPayload=await linkResponse.json();if(!linkResponse.ok)throw new Error(linkPayload.error||'تعذر ربط المشروع بمشروع الري.');
      const syncResponse=await fetch('/api/irrigation/sync',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({workProjectId:projectId})});const syncPayload=await syncResponse.json();if(!syncResponse.ok)throw new Error(syncPayload.error||'تعذرت مزامنة مواقع مشروع الري.');
      const sitesResponse=await fetch(`/api/sites?t=${Date.now()}`,{cache:'no-store'});const sitesPayload=await sitesResponse.json();if(!sitesResponse.ok)throw new Error(sitesPayload.error||'تعذر قراءة المواقع بعد المزامنة.');
      const localSites=((sitesPayload.sites||[]) as LocalSite[]).filter(site=>site.project_id===projectId&&site.source_site_id);const localSiteBySource=new Map(localSites.map(site=>[site.source_site_id as string,site.id]));
      ...
      for(const order of orders){
        ...
        const localSiteIds=order.siteIds.map(sourceId=>localSiteBySource.get(sourceId)).filter((siteId):siteId is string=>Boolean(siteId));
        if(localSiteIds.length!==order.siteIds.length)throw new Error(`تعذر تحويل جميع مواقع أمر العمل ${order.number} إلى مواقع محلية بعد المزامنة.`);
        if(localSiteIds.length){const linked=await supabase.from('work_order_sites').insert(localSiteIds.map(siteId=>({work_order_id:workOrderId,site_id:siteId})));if(linked.error)throw linked.error;}
      ...
      setDone(true);setStatus(`تم اعتماد المشروع وربطه ومزامنة ${syncPayload.synced??0} موقع وإنشاء أوامر العمل بنجاح.`);
  ...
  return <main className="page import-page manual-import-page">
    ...
    {data?<>
      ...
      <section className="manual-orders-setup"><header><div><span className="section-kicker">الإدخال اليدوي</span><h2>كم عدد أوامر العمل؟</h2><p>بعد تحديد العدد سيظهر نموذج مستقل لكل أمر، مع اختيار مشروع الري ومواقعه.</p>{catalogLoading?<small className="catalog-state">جاري جلب مشاريع الري ومواقعها تلقائيًا...</small>:catalogError?<button type="button" className="catalog-retry" onClick={()=>void loadIrrigationCatalog()}>{catalogError} — إعادة المحاولة</button>:<small className="catalog-state success">تم جلب {irrigationProjects.length} مشروع ري و{irrigationProjects.reduce((sum,project)=>sum+project.sites.length,0)} موقع.</small>}</div><input type="number" min="0" max="50" value={orders.length||''} placeholder="0" onChange={e=>setOrderCount(Number(e.target.value))}/></header></section>
      {orders.length?<section className="manual-orders-workspace"><nav>{orders.map((order,index)=><button key={index} className={activeOrder===index?'active':''} onClick={()=>setActiveOrder(index)}><b>أمر {order.number}</b><small>{validation[index]?.ready?'مكتمل ✓':'يحتاج إدخال'}</small></button>)}</nav>{active?<div className="manual-order-editor"><div className="manual-order-fields"><label>رقم الأمر<input value={active.number} onChange={e=>updateOrder(activeOrder,{number:e.target.value})}/></label><label>تاريخ البداية<input type="date" value={active.startDate} onChange={e=>updateOrder(activeOrder,{startDate:e.target.value})}/></label><label>تاريخ النهاية<input type="date" value={active.endDate} onChange={e=>updateOrder(activeOrder,{endDate:e.target.value})}/></label><div className="manual-order-value"><small>قيمة التنفيذ المحسوبة</small><b>{activeValue.toLocaleString('en-US',{maximumFractionDigits:2})}</b></div></div>
        <div className="manual-sites-picker"><header><div><h3>مشروع الري والمواقع المشمولة</h3><p>اختر مشروع الري أولًا؛ ستظهر مواقعه مباشرة من المصدر.</p></div><b>{active.siteIds.length} مختارة</b></header><label className="irrigation-project-choice"><span>مشروع الري</span><select value={active.irrigationProjectId} onChange={e=>chooseIrrigationProject(activeOrder,e.target.value)}><option value="">اختر المشروع...</option>{irrigationProjects.map(project=><option key={project.id} value={project.id}>{project.name} ({project.sites.length} موقع)</option>)}</select></label>{catalogLoading?<div className="empty">جاري جلب مشاريع الري والمواقع...</div>:catalogError?<div className="empty">تعذر جلب الدليل. استخدم إعادة المحاولة في الأعلى.</div>:!active.irrigationProjectId?<div className="empty">اختر مشروع الري لعرض المواقع التابعة له.</div>:activeIrrigationProject?.sites.length?<div>{activeIrrigationProject.sites.map(site=><label className={active.siteIds.includes(site.id)?'selected':''} key={site.id}><input type="checkbox" checked={active.siteIds.includes(site.id)} onChange={()=>toggleSite(activeOrder,site.id)}/><span>✓</span><b>{site.name}</b><small>{site.district||site.code||'موقع نشط في نظام الري'}</small></label>)}</div>:<div className="empty">لا توجد مواقع نشطة ضمن مشروع الري المختار.</div>}</div>
      ...
      {orders.length?<section className="manual-import-approval"><div><span className="section-kicker">الاعتماد النهائي</span><h2>{ready?'البيانات جاهزة للاعتماد':'أكمل بيانات أوامر العمل'}</h2><p>{ready?`${orders.length} أوامر مكتملة، وسيتم إنشاء المشروع وربطه بمشروع الري ومزامنة المواقع ثم إنشاء البنود والعلاقات.`:'كل أمر يحتاج مشروع ري، وتاريخ بداية ونهاية، وموقعًا واحدًا على الأقل، وكمية منفذة لبند واحد على الأقل.'}</p></div><button className="btn primary" disabled={!ready||saving||done} onClick={()=>void approve()}>{saving?'جاري الربط والمزامنة والاعتماد...':done?'تم الاعتماد':'اعتماد استيراد المشروع'}</button></section>:null}
    </>:null}
  </main>;
}
