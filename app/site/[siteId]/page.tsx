...
export default function SitePage({ params }: { params: { siteId: string } }) {
  ...
  async function load(){
    ...
    if(ordersResult.error || linesResult.error){ setMessage(ordersResult.error?.message || linesResult.error?.message || 'تعذر تحميل أوامر العمل.'); setLoading(false); return; }
  ...
  const totalExecuted=lines.reduce((s,l)=>s+num(l.executed_quantity ?? l.quantity),0);
  ...
  return <main className="page site-story-page">
    ...
    <div className="stats site-story-stats">
      ...
      <div className="stat"><strong>{fmt(totalExecuted)}</strong><span>إجمالي كميات سجلات الأوامر</span></div>
    ...
    <div className="notice site-story-note">ملاحظة: ملف Excel الحالي يربط البند بأمر العمل، ويربط أمر العمل بالمواقع. لذلك الكميات أدناه هي كميات <b>أمر العمل المرتبط بالموقع</b>، وليست توزيعًا مؤكدًا للكمية على هذا الموقع وحده عندما يكون الأمر مرتبطًا بأكثر من موقع.</div>
      ...
      <div className="story-timeline">
        {filteredOrders.map((o,index)=>{ const orderLines=linesByOrder.get(o.id)||[]; const executed=orderLines.reduce((s,l)=>s+num(l.executed_quantity ?? l.quantity),0); return <article className="story-order" key={o.id}>
          ...
          <div className="story-order-card">
            <div className="story-order-head"><div><span className="badge">أمر عمل {o.work_order_number}</span><h3>{o.work_order_date || 'تاريخ غير مسجل'}</h3><p className="muted">{o.title || ''}{o.status ? ` · ${o.status}` : ''}</p></div><div className="story-order-total"><small>إجمالي كميات السجلات</small><b>{fmt(executed)}</b><span>{orderLines.length} بند</span></div></div>
            <div className="story-lines">{orderLines.map(l=><div className="story-line" key={l.id}><div><b>{itemMap.get(l.item_id)?.name || `بند ${l.item_no||''}`}</b><small>{l.unit || 'بدون وحدة'}</small></div><span><small>المنفذ</small><b>{fmt(l.executed_quantity ?? l.quantity)}</b></span><span><small>المتبقي بعد الأمر</small><b>{fmt(l.remaining_quantity)}</b></span></div>)}</div>
          </div>
        </article>})}
    ...
    <section className="panel site-story-section">
      <div className="section-title"><div><span className="section-kicker">الملخص التراكمي</span><h2>البنود التي مرّت على الموقع</h2></div><span>{cumulative.length} بند</span></div>
      <div className="table-wrap"><table><thead><tr><th>البند</th><th>الوحدة</th><th>عدد أوامر العمل</th><th>إجمالي المنفذ في سجلات الأوامر</th><th>آخر متبقٍ مسجل</th></tr></thead><tbody>{cumulative.map((x,i)=><tr key={i}><td><b>{x.name}</b></td><td>{x.unit}</td><td>{x.orders.size}</td><td>{fmt(x.executed)}</td><td>{fmt(x.latestRemaining)}</td></tr>)}</tbody></table></div>
    </section>
  </main>;
}
